...
const SPRAY_MAT := preload('res://addons/ocean_waves_fft/mat_spray.tres')
# high-клипмап не вендорится (14 МБ и не используется: сетку даёт OceanEnvironment,
# клипмап библиотеки — квадрат всего 512 м). Обе константы смотрят на образец low.
const WATER_MESH_HIGH := preload('res://addons/ocean_waves_fft/clipmap_low.obj')
const WATER_MESH_LOW := preload('res://addons/ocean_waves_fft/clipmap_low.obj')
