...
## Returns a [PackedFloat32Array] from the provided data.
##
## ПОРТИРОВАНИЕ НА GODOT 4.7 (правка против оригинала). Здесь размер округлялся вверх до
## кратного 16. В 4.3 это проходило, а 4.7 сверяет размер push-константы с тем, что
## объявил шейдер, ТОЧНО, и падал с «requires (52) bytes, supplied (64)». Паддинг убран:
## без него размеры совпадают ровно (13/5/1 float → 52/20/4 байта), а требование Vulkan
## «кратно 4» держится само собой, потому что элементы четырёхбайтовые.
