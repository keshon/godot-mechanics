...
func init_gpu(num_cascades : int) -> void:
	...
	var spectrum_compute_shader := context.load_shader('res://addons/ocean_waves_fft/shaders/compute/spectrum_compute.glsl')
	var fft_butterfly_shader := context.load_shader('res://addons/ocean_waves_fft/shaders/compute/fft_butterfly.glsl')
	var spectrum_modulate_shader := context.load_shader('res://addons/ocean_waves_fft/shaders/compute/spectrum_modulate.glsl')
	var fft_compute_shader := context.load_shader('res://addons/ocean_waves_fft/shaders/compute/fft_compute.glsl')
	var transpose_shader := context.load_shader('res://addons/ocean_waves_fft/shaders/compute/transpose.glsl')
	var fft_unpack_shader := context.load_shader('res://addons/ocean_waves_fft/shaders/compute/fft_unpack.glsl')
	...
	# ПРАВКА WORLDBOX (проба 23): добавлен CAN_COPY_FROM_BIT. Без него карту смещений
	# нельзя снять на CPU вообще — texture_get_data отказывается. А без этого физика
	# не может спросить у FFT высоту в точке. Флаг ничего не стоит, пока не читаешь.
	...
	# ПОРТИРОВАНИЕ НА GODOT 4.7 (правка против оригинала). Один и тот же набор нельзя
	# переиспользовать между шейдерами, которые объявляют ресурс С РАЗНЫМ ДОСТУПОМ: в 4.3
	# это проходило, 4.7 сверяет writable-флаги и падает с «not the same format as required
	# by the pipeline shader». А расходятся они по делу: `spectrum` — writeonly в
	# spectrum_compute и readonly в spectrum_modulate; `fft_buffer` — writeonly в
	# spectrum_modulate и на чтение-запись в fft_unpack. Поэтому набор теперь свой на
	# каждую пару (шейдер, номер набора).
	...
	pipelines[&'spectrum_modulate'] = context.create_pipeline([map_size/16, map_size/16, 1], [spectrum_set_ro, fft_buffer_set], spectrum_modulate_shader)
	...
	pipelines[&'fft_unpack'] = context.create_pipeline([map_size/16, map_size/16, 1], [unpack_set, fft_buffer_set_unpack], fft_unpack_shader)
