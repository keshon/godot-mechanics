...
##
## В 37-й всего этого не было и не могло быть: там тяга сразу превращалась в силу на
## колесе. Здесь мотор — отдельное тело со своей инерцией, и оно может крутиться БЫСТРЕЕ
...
##
## Сцепление — не выключатель, а пружина по скорости: момент пропорционален разнице
## оборотов и ограничен сверху. Отсюда бесплатно берётся и трогание с места, и провал
## оборотов под нагрузкой, и то, что мотор на нейтрали свободно раскручивается.
...
## Предельный момент, который сцепление вообще способно передать, Н·м. Ниже момента
## мотора — и оно будет буксовать вместо колёс.
var clutch_capacity := 480.0
var clutch_stiffness := 3.2

## Угловая скорость мотора, рад/с.
var omega := IDLE * TAU / 60.0
var gear := 1
...
static func split(
	...
	var difference := omega_left - omega_right
	var cap := absf(total) * 0.5 + bias_preload
	var bias := clampf(difference * lock, -cap, cap)
	bias += signf(difference) * bias_preload
...
func torque_at(revs: float, throttle: float) -> float:
	if revs > REDLINE:
		return 0.0
	var shape := clampf(1.0 - absf(revs - peak_rpm) / peak_rpm * 0.55, 0.45, 1.0)
	...
	var idle_hold := maxf(IDLE - revs, 0.0) * 0.06
	var pumping := -0.014 * revs * (1.0 - throttle)
	return drive + idle_hold + pumping


## Один шаг трансмиссии. На входе — газ и средняя угловая скорость ведущих колёс; на
## выходе момент, приходящий НА КОЛЁСА (уже умноженный на передаточное число).
func step(delta: float, throttle: float, wheel_omega: float) -> float:
	...
	var mismatch := omega - wheel_omega * geared
	clutch_torque = clampf(
			mismatch * clutch_stiffness, -clutch_capacity, clutch_capacity)
	var engine := torque_at(rpm(), throttle)
	omega = maxf(omega + (engine - clutch_torque) / engine_inertia * delta, 0.0)
	return clutch_torque * geared
...
func _shift(wheel_omega: float) -> void:
	var revs := rpm()
	if revs > REDLINE * 0.87 and gear < gears.size():
		gear += 1
		omega = wheel_omega * ratio()
	elif revs < IDLE * 2.3 and gear > 1:
		gear -= 1
