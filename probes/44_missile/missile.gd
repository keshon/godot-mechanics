...
##
## The stick springs back to centre when let go. Adding mouse movement to an accumulator
## that nothing ever returns leaves any net drift of the hand as a permanently deflected fin
## and a permanently turning missile.
##
...
## Seconds after which a row is called a failure rather than a turn.
const GIVE_UP := 10.0
## High enough that a failed turn ends in a timeout, not in the ground. Metres.
const TEST_ALT := 2500.0
## How fast the camera offset catches up, per second.
const TRAIL_RATE := 6.0
const SUSTAIN_THRUST := 1400.0
...
## Distance the camera trails behind the missile, metres.
...
@onready var _missile: MissileBody = $Body
@onready var _rail: Node3D = $Rail


func _ready() -> void:
	Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
	_missile.struck.connect(_on_missile_struck)
	var gates := get_tree().get_nodes_in_group("gate")
	_gate_total = gates.size()
	for gate in gates:
		(gate as Area3D).body_entered.connect(_on_gate_body_entered)
	_park()
...
func _physics_process(delta: float) -> void:
	...
	if _missile.flying:
		_flight += delta
		_peak_g = maxf(_peak_g, _missile.lateral_g)
		_missile.pitch_input = _pitch
		_missile.yaw_input = _yaw
		_missile.roll_input = _roll
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if key != null and key.pressed and not key.echo:
		...
		if not _missile.flying and _row < 0:
			_yaw = 0.0
			_pitch = 0.0
			_roll = 0.0
			_missile.launch(
					_rail.global_transform, -_rail.global_basis.z * launch_speed)


func _apply_key(keycode: Key) -> void:
	match keycode:
		KEY_1:
			_missile.limiter = not _missile.limiter
		KEY_2:
			_missile.sustain_thrust = (
					0.0 if _missile.sustain_thrust > 0.0 else SUSTAIN_THRUST)
		KEY_3:
			_chase = not _chase
		KEY_G:
			_table.clear()
...
func _park() -> void:
	_missile.flying = false
	_missile.freeze = true
	_missile.global_transform = _rail.global_transform
	_missile.linear_velocity = Vector3.ZERO
	_missile.angular_velocity = Vector3.ZERO
	_missile.pitch_input = 0.0
	_missile.yaw_input = 0.0
	_missile.roll_input = 0.0
	_yaw = 0.0
	_pitch = 0.0
	_roll = 0.0
	_gates = 0
...
func _follow(delta: float) -> void:
	if not _chase:
		_camera.look_at(_missile.global_position, Vector3.UP)
		return
	var back := _missile.global_basis.z * chase_back + Vector3.UP * chase_up
	_trail = (back if _trail == Vector3.ZERO
			else _trail.lerp(back, clampf(delta * TRAIL_RATE, 0.0, 1.0)))
	_camera.global_position = _missile.global_position + _trail
	_camera.look_at(
			_missile.global_position + -_missile.global_basis.z * 30.0, Vector3.UP)
...
func _measure(delta: float) -> void:
	_elapsed += delta
	_missile.pitch_input = 1.0
	_missile.yaw_input = 0.0
	var velocity := _missile.linear_velocity
	_arc += velocity.length() * delta
	_peak_g = maxf(_peak_g, _missile.lateral_g)
	...
	_sum_g += _missile.lateral_g
	_ticks += 1
	var turned := (0.0 if velocity.length() < 1.0
			else _from.angle_to(velocity.normalized()))
	_best = maxf(_best, turned)
	var done := turned >= QUARTER
	if not done and _elapsed <= GIVE_UP and _missile.flying:
		return
	_close_row(velocity, done)


func _close_row(velocity: Vector3, done: bool) -> void:
	_table.append({
		"speed": _entry,
		"time": _elapsed,
		"exit": velocity.length(),
		...
		# answer comes out as the ratio of entry speeds while looking like a measurement.
		"radius": _arc / maxf(_best, 0.0001),
		"peak": _peak_g,
		"held": _sum_g / maxf(_ticks, 1),
		"turned": rad_to_deg(_best),
		"ok": done,
		"why": "" if done else ("земля" if not _missile.flying else "время"),
	})
	_row += 1
	if _row < SPEEDS.size():
		_start_row()
...
func _start_row() -> void:
	...
	at.origin = Vector3(0.0, TEST_ALT, 0.0)
	_missile.launch(at, -at.basis.z * speed)
	# Motor dead on purpose: this measures the airframe, not the rocket behind it.
	_missile.fuel = 0.0
	_from = (-at.basis.z).normalized()
	_entry = speed
	_elapsed = 0.0
	_peak_g = 0.0
	_sum_g = 0.0
...
func _readout() -> void:
	var phase := "на рельсе"
	if _missile.flying:
		if _missile.fuel <= 0.0:
			phase = "выбег"
		elif _missile.burn < _missile.boost_time:
			phase = "разгон"
		else:
			phase = "маршевый"
	var lines := PackedStringArray([
		"[b]РАКЕТА[/b]   %s   топливо %.0f%%   ворот пройдено %d из %d" % [
			phase, _missile.fuel * 100.0, _gates, _gate_total],
		"скорость [b]%.0f[/b] м/с   перегрузка [b]%.1f[/b] g   пик %.1f   угол атаки %.1f°" % [
			_missile.speed, _missile.lateral_g, _peak_g, rad_to_deg(_missile.alpha)],
		"крен %.0f °/с   тангаж %.0f °/с   рыскание %.0f °/с" % [
			rad_to_deg(_missile.angular_velocity.dot(-_missile.global_basis.z)),
			rad_to_deg(_missile.angular_velocity.dot(_missile.global_basis.x)),
	...
	if _row >= 0:
		lines.append("")
		lines.append("[color=#66ccff]замер: %.0f м/с, %.1f с[/color]" % [_entry, _elapsed])
...
func _append_table(lines: PackedStringArray) -> void:
	...
	lines.append("[b]вираж на 90° с мёртвым мотором, ограничитель %s[/b]" % (
		"вкл" if _missile.limiter else "выкл"))
	...
	for row in _table:
		lines.append("[cell]%.0f м/с[/cell][cell]%s[/cell][cell]%s[/cell]"
			% [row["speed"], "%.2f с" % row["time"] if row["ok"] else "—",
				"%.0f м" % row["radius"] if row["ok"]
				else "дошла до %.0f° (%s)" % [row["turned"], row["why"]]]
			+ "[cell]%.0f м/с[/cell][cell]%.1f / %.1f[/cell]" % [
				row["exit"], row["held"], row["peak"]])
	lines.append("[/table]")
...
func _on_missile_struck(_where: Vector3, _speed: float) -> void:
	if _row < 0:
		_missile.pitch_input = 0.0
		_missile.yaw_input = 0.0
