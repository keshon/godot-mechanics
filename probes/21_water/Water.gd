...
#
# One shader, six switches, three presets. The presets are nothing but numbers: the
...
#
# The second half of the probe is the part MGS2 taught: what makes water convincing is
# often not the water. Dive under and the motes and the wobble do more than the surface
# shader ever did.
...
const PRESETS := {
	"ocean": {
		...
		"refract_amount": 0.35, "water_rough": 0.03, "murk": 0.55, "fog_col": Color(0.05, 0.24, 0.34),
	...
	"river": {
		...
		"refract_amount": 0.6, "water_rough": 0.09, "murk": 0.8, "fog_col": Color(0.14, 0.22, 0.12),
	...
	"pool": {
		...
		"refract_amount": 0.9, "water_rough": 0.01, "murk": 0.18, "fog_col": Color(0.22, 0.55, 0.66),
...
const LAYERS := ["use_waves", "use_ripple", "use_depth", "use_fresnel",
	"use_refract", "use_foam"]
const LAYER_KEYS := ["4", "5", "6", "7", "8", "9"]

@export var fly_speed := 9.0

var preset := "ocean"
var on := [true, true, true, true, true, true]
var wet := 0.0
var under := false
var show_depth := false

var _fog_col := Color(0.08, 0.28, 0.36)
var _fog_den := 0.06
var _vol := 0.0
...
func _ready() -> void:
	_mat = ($Water as MeshInstance3D).material_override
	_screen = ($Ui/Wet as ColorRect).material
	apply_preset("ocean")
...
func apply_preset(name: String) -> void:
	...
	for k in p:
		if not SCENE_KEYS.has(k):
			_mat.set_shader_parameter(k, p[k])
	_screen.set_shader_parameter("murk", float(p["murk"]) * 0.45)
	_fog_den = 0.012 + 0.085 * float(p["murk"])
	_fog_col = p["fog_col"]
	($Particles as GPUParticles3D).amount = int(p["motes"])
	_vol = float(p["vol"])
	$Cam.global_position = p["at"]
	_yaw = 0.0
	_pitch = -0.12 if name != "flooded" else 0.08
	_push_layers()


func _push_layers() -> void:
	for i in LAYERS.size():
		_mat.set_shader_parameter(LAYERS[i], 1.0 if on[i] else 0.0)
...
func _process(delta: float) -> void:
	...
	var was := under
	under = cam.global_position.y < 0.0
	if was and not under:
		wet = 1.0                      # just surfaced: the lens is covered
	wet = maxf(wet - delta * 0.28, 0.0)
	_screen.set_shader_parameter("submerged", 1.0 if under else 0.0)
	...
	env.fog_enabled = under
	# volumetric fog is what turns a spotlight into a shaft of light in the water.
	# it is not a water effect at all, and it does more for the room than the shader.
	env.volumetric_fog_enabled = under and _vol > 0.0
	if under:
		env.fog_light_color = _fog_col
		env.fog_density = _fog_den
		env.volumetric_fog_density = _vol
		env.volumetric_fog_albedo = _fog_col.lightened(0.45)
		env.volumetric_fog_length = 40.0
	_screen.set_shader_parameter("drops", wet)
	...
	# AHEAD of where you are looking and nothing can get between you and the near plane.
	var motes := $Particles as GPUParticles3D
	motes.visible = under
	motes.emitting = under
	var ahead: Vector3 = cam.global_transform.basis * Vector3(0.0, 0.0, -7.0)
	motes.global_position = Vector3(
		cam.global_position.x + ahead.x,
		clampf(cam.global_position.y + ahead.y, -12.0, -1.0),
		cam.global_position.z + ahead.z)
	_hud()
	...
	if event is InputEventMouseButton:
		...
		match event.keycode:
			...
			KEY_U:
				...
				_mat.set_shader_parameter("show_depth", 1.0 if show_depth else 0.0)



func _hud() -> void:
	var t := "[b]%s[/b]    1 ocean   2 river   3 pool   F flooded room\n" % preset.to_upper()
	t += "absorb %.1f m — how far light gets before the water gives up\n\n" % float(
		PRESETS[preset]["absorb"])
	for i in LAYERS.size():
		var name: String = str(LAYERS[i]).replace("use_", "")
		t += "%s  %-9s %s\n" % [LAYER_KEYS[i], name,
			"[color=#7fe08a]on[/color]" if on[i] else "[color=#666666]off[/color]"]
	...
	if wet > 0.0:
		t += "[color=#ffd479]drops on the lens %.0f%%[/color]\n" % (wet * 100.0)
	t += "\nWASD fly   SPACE up   SHIFT down   RIGHT MOUSE look\n"
	t += "swim down past y=0 and look up"
	$Ui/Info.text = t
