...
## а предел проекции скорости на желаемое направление, м/с.
@export var air_wish_speed := 1.2
## ГРАВИТАЦИЯ РАЗНАЯ НА ПОДЪЁМЕ И НА ПАДЕНИИ. Одинаковая читается как «тяжело»: прыжок
## одновременно низкий и долгий, и добавлять высоту приходится задиранием импульса,
## отчего он становится ещё дольше. Разведённые — подъём лёгкий, падение быстрое — дают
## высокий И резкий прыжок сразу. Приём из платформеров, в шутерах он тот же.
@export var gravity := 18.0
@export var gravity_fall := 32.0
@export var jump_speed := 7.2

@export_group("Прощение")
## Две форточки, которые есть в каждом платформере и которых не видит никто. Секунды.
...
## рывок перестаёт быть рывком: он становится сменой режима полёта. «Длинный и
## воздушный» — это ровно оно.
@export var dash_exit_speed := 14.0
## Рад/с доворота рывка. Воздушное управление во время рывка формально работает, но толку
## не даёт: прибавка ограничена 1.2 м/с, а скорость тридцать — проекция всегда выше
## потолка, и прибавка уходит в минус. Значит рывок надо ПОВОРАЧИВАТЬ, сохраняя модуль, а
## не разгонять. Ноль — рывок строго по прямой, как было.
...
## Горизонтальная скорость, м/с. Табло читает её каждый кадр.
var speed := 0.0
var state := "земля"
## 0 — рывок только что потрачен, 1 — готов.
var dash_ready := 1.0
## Сколько прыжков в воздухе осталось. Публично: табло показывает это игроку.
...
@onready var _stand_height: float = (_collider.shape as CapsuleShape3D).height
...
func _ready() -> void:
	...
	_stand_height = (_collider.shape as CapsuleShape3D).height


func _physics_process(delta: float) -> void:
	_clock += delta
	_dash_cooldown_left = maxf(_dash_cooldown_left - delta, 0.0)
	dash_ready = 1.0 - _dash_cooldown_left / dash_cooldown

	if _mantle_left > 0.0:
		_advance_mantle(delta)
		return
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if not event.pressed and event.physical_keycode == KEY_SPACE:
...
## прибавка это разница между потолком и текущей проекцией. Летишь строго вперёд —
## проекция равна потолку, добавить нечего. Летишь боком, а просишь вперёд — проекция
## около нуля, и тебе дают полную порцию. Из этих трёх строк выросли strafe-jumping и
## bunny hop.
func _accelerate(
		direction: Vector3, wish_speed: float, accel: float, delta: float) -> void:
	if direction == Vector3.ZERO:
		return
	var current := velocity.dot(direction)
	var room := wish_speed - current
	if room <= 0.0:
		return
	velocity += direction * minf(accel * wish_speed * delta, room)
...
func _on_ground(wish: Vector3, delta: float) -> void:
	...
	if _dash_left > 0.0:
		...
		velocity.y = -2.0
		return
	var friction := slide_friction if _sliding else ground_friction
	var flat := Vector3(velocity.x, 0.0, velocity.z)
	var along := flat.length()
	if along > 0.0:
		# ТРЕНИЕ ОТДЕЛЬНО ОТ РАЗГОНА. Слитые в один `move_toward` они дают ватное
		# управление: отпустил клавишу — и едешь по инерции ровно так же, как
		# разгоняешься.
...
func _in_air(wish: Vector3, delta: float) -> void:
	...
	if coyote and _since_ground < coyote_time:
		_jump()
	elif double_jump and air_jumps_left > 0:
		_air_jump()
...
##
## Первая: вертикальная скорость СБРАСЫВАЕТСЯ, а не складывается. Падая на десяти метрах
## в секунду, ты иначе получишь почти ничего — и решишь, что кнопка не сработала.
##
## Вторая: горизонтальный импульс ДОВОРАЧИВАЕТСЯ к тому, куда ты просишь. Наполовину, не
## полностью: полный доворот убивает наказание за плохой заход, отсутствие доворота
## делает второй прыжок бесполезным при промахе. Половина — «исправить, но не переиграть».
func _air_jump() -> void:
	air_jumps_left -= 1
	_jump_used = true
	...
	if wish == Vector3.ZERO:
		return
	if along < 0.5:
		velocity.x = wish.x * ground_speed * 0.6
		velocity.z = wish.z * ground_speed * 0.6
		return
	var turned := flat.normalized().lerp(wish, 0.5).normalized() * along
	velocity.x = turned.x
	velocity.z = turned.z
...
func _steer_dash(wish: Vector3, delta: float) -> void:
	...
	if angle < 0.0001:
		return
	var turned := facing.slerp(wish, minf(dash_steer * delta / angle, 1.0)).normalized()
	velocity.x = turned.x * along
	velocity.z = turned.z * along


func _start_dash() -> void:
	if not dash or _dash_cooldown_left > 0.0:
		return
	var wish := _wish_direction()
	if wish == Vector3.ZERO:
		wish = Vector3(-sin(_yaw), 0.0, -cos(_yaw))
	# Рывок ЗАДАЁТ скорость, а не добавляет её. Прибавка складывалась бы с разгоном, и
	# цепочка рывков выкидывала бы игрока за карту; заданная скорость — это ещё и
	# обещание «рывок всегда одинаковый», на которое можно опереться в прыжке через
	# пропасть.
	velocity.x = wish.x * dash_speed
	velocity.z = wish.z * dash_speed
...
func _end_dash() -> void:
	var flat := Vector3(velocity.x, 0.0, velocity.z)
	var along := flat.length()
	if along <= dash_exit_speed:
		return
	flat = flat / along * dash_exit_speed
	velocity.x = flat.x
	velocity.z = flat.z
...
func _run_wall(wish: Vector3, delta: float) -> void:
	...
	# Скорость ПРОЕЦИРУЕТСЯ на стену, а не гасится: смысл бега по стене в том, что
	# импульс сохраняется. Гасить его — значит сделать красивую анимацию, которая мешает
	# бежать.
	var along := velocity - _wall_normal * velocity.dot(_wall_normal)
	velocity.x = along.x
	velocity.z = along.z
	var ahead := along
	ahead.y = 0.0
	if ahead.length() > 0.1:
		_accelerate(ahead.normalized(), ground_speed * 1.15, ground_accel * 0.5, delta)
	if not _wants_jump():
		return
	# Отскок ОТ стены плюс вверх. Только вверх — и стена становится лестницей; только от
	# стены — и с неё невозможно уйти вперёд.
	velocity += _wall_normal * wall_jump_out
...
## головы — стены нет, сверху вниз — есть на что встать. Без второго игрок
## «подтягивается» на глухую стену; без третьего — в воздух.
func _try_mantle() -> bool:
	...
	var at_waist := space.intersect_ray(PhysicsRayQueryParameters3D.create(
			waist, waist + ahead * 1.0, 1, [get_rid()]))
	if at_waist.is_empty():
		return false
	var head := global_position + Vector3.UP * 1.85
	var at_head := space.intersect_ray(PhysicsRayQueryParameters3D.create(
			head, head + ahead * 1.0, 1, [get_rid()]))
	if not at_head.is_empty():
		return false
	var probe: Vector3 = at_waist["position"] + ahead * 0.35 + Vector3.UP * 1.6
	var top := space.intersect_ray(PhysicsRayQueryParameters3D.create(
			probe, probe - Vector3.UP * 1.9, 1, [get_rid()]))
	if top.is_empty():
		return false
	var lift: float = top["position"].y - global_position.y
	if lift < 0.35 or lift > 1.7:
		return false
	_mantle_from = global_position
	_mantle_to = top["position"] + ahead * 0.35 + Vector3.UP * 0.05
	_mantle_left = mantle_time
	return true


func _advance_mantle(delta: float) -> void:
	state = "уступ"
	_mantle_left -= delta
	var done := clampf(1.0 - _mantle_left / mantle_time, 0.0, 1.0)
	# Подъём раньше, доводка вперёд позже: иначе игрок въезжает в угол уступа и застревает.
	global_position = Vector3(
			lerpf(_mantle_from.x, _mantle_to.x, done * done),
			lerpf(_mantle_from.y, _mantle_to.y, sqrt(done)),
			lerpf(_mantle_from.z, _mantle_to.z, done * done))
...
func _start_slide() -> void:
	...
	capsule.height = _stand_height * 0.5
	_collider.position.y = -_stand_height * 0.25


func _stop_slide() -> void:
	if not _is_ceiling_free():
		return
	_sliding = false
	var capsule := _collider.shape as CapsuleShape3D
	capsule.height = _stand_height
	_collider.position.y = 0.0
...
func _is_ceiling_free() -> bool:
	var space := get_world_3d().direct_space_state
	var from := global_position + Vector3.UP * 0.1
	var query := PhysicsRayQueryParameters3D.create(
			from, from + Vector3.UP * (_stand_height * 0.5 + 0.15), 1, [get_rid()])
...
## имитирует оптический поток — то, как мир проносится навстречу; боковое движение такого
## потока не даёт, и рывок стрейфом раздувал картинку без всякой причины. Заодно
## бесплатно решается движение спиной вперёд: проекция отрицательна, кика нет.
func _drive_camera(delta: float) -> void:
	var ahead := Vector3(-sin(_yaw), 0.0, -cos(_yaw))
	var closing := Vector3(velocity.x, 0.0, velocity.z).dot(ahead)
	var wanted_fov := 78.0 + clampf(closing - ground_speed, 0.0, 16.0) * 1.5
	_camera.fov = lerpf(_camera.fov, wanted_fov, 1.0 - exp(-8.0 * delta))
	var wanted_roll := 0.0
	if _wall_left > 0.0:
		var side := _wall_normal.cross(Vector3.UP).dot(ahead)
		wanted_roll = wall_roll * signf(side)
	_roll = lerpf(_roll, wanted_roll, 1.0 - exp(-9.0 * delta))
	_camera.rotation = Vector3(_pitch, 0.0, _roll)
	rotation.y = _yaw
	var duck := -0.45 if _sliding else 0.0
	_camera.position.y = lerpf(_camera.position.y, 0.65 + duck, 1.0 - exp(-14.0 * delta))
