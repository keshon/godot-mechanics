...
## under the block is built from that same number. An RD-107 is one engine with four
## chambers, which is why a Soyuz has four bells under each carrot and four more under the
## core — change `engines` and both the number and the thrust follow, because they are the
## same fact written once.
##
## The ring is the one thing here built by code rather than dragged into place, and that is
## the case the rules name outright: many of a thing, arranged by a rule, not touched by hand.
...
## Sea-level thrust of ONE chamber.
@export_range(0.0, 1000000.0, 250.0) var chamber := 98750.0
## Radius of the circle the bells sit on.
@export_range(0.0, 4.0, 0.01) var ring := 0.62
## Exhaust speed. Divided by the surface gravity it is the specific impulse in seconds, and
## it is the only thing that decides how far a given tank of fuel can throw a given mass.
@export_range(500.0, 5000.0, 10.0) var exhaust := 2600.0
## How far the engines can swivel. Below the atmosphere this is the only control there is.
...
## Lit on the pad, whatever group it belongs to. This is what an R-7 is: the core does not
## wait for the boosters to go, it burns with them from the first second and keeps burning
## after they leave. Written as a flag rather than assumed, because most rockets are not
## built that way and the difference is the whole character of the vehicle.
...
func _build() -> void:
	...
	for i in engines:
		...
		var a := TAU * float(i) / float(engines)
		one.position = Vector3(cos(a), sin(a), 0.0) * (ring if engines > 1 else 0.0)
...
## WHERE THE NOZZLES ARE, along the stack's own axis. The thrust is applied there, and the
## distance from that plane to the centre of mass is the entire lever the gimbal works with.
## Taken from the mount node rather than written down: move the engines in the editor and the
## rocket's authority over itself changes with them.
func nozzle() -> float:
	return position.z + (_mount.position.z * scale.z if _mount != null else 0.0)


## WHERE THE PLUMES POINT. The body writes it, the nodes turn, and the gimbal stops being a
## number in the readout. Below the atmosphere the engines are the only control there is, so
## a probe that never shows them moving is hiding its own subject.
...
func burn(throttle: float, delta: float, air: float) -> float:
	...
	var alight := lit and fuel > 0.0 and throttle > 0.01
	for f in _flames:
		f.visible = alight
		f.scale = Vector3.ONE * (0.35 + 0.65 * ramp)
	if not alight:
		return 0.0
	since_light += delta
