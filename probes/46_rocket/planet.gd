...
##
## Small on purpose. A real Earth means eight kilometres per second and nine minutes of ascent,
## and the probe would be a waiting simulator; a hundred and twenty kilometres of radius means
## orbit at eleven hundred metres per second and two minutes of work. The scale is a knob.
##
...
## Distance from the centre to the pad, metres. Orbital speed goes as its square root, so
## this one number sets the pace of the whole probe.
...
## Height over which the air thins by a factor of e, metres. Everything about a gravity turn
## is decided by this number against the burn time.
...
func down(at: Vector3) -> Vector3:
	...
	return inward.normalized() if inward.length_squared() > 0.0 else Vector3.DOWN


func gravity(at: Vector3) -> Vector3:
	var range_to_centre := maxf(at.distance_to(centre), 1.0)
	# Inverse square, normalised so that the surface reads exactly the stated number.
	return (down(at) * surface_gravity * (radius * radius)
			/ (range_to_centre * range_to_centre))


func density(at: Vector3) -> float:
	var height := altitude(at)
	if height >= edge:
		return 0.0
	return sea_density * exp(-maxf(height, 0.0) / scale_height)
...
func orbital_speed(height: float) -> float:
	var range_to_centre := radius + maxf(height, 0.0)
	return sqrt(surface_gravity * radius * radius / range_to_centre)
