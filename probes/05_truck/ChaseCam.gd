...
func _physics_process(delta: float) -> void:
	var want: Vector3
	var look: Vector3 = _target.global_position
	if view == View.CHASE:
		...
		look = _target.global_position + Vector3(0, 0.4, 0)
	var k: float = chase_damp if view == View.CHASE else side_damp
	global_position = want + (global_position - want) * exp(-k * delta)
