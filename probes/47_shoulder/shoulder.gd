...
## stopped working. Measuring both over one set of ranges hides that behind a column of misses.
...
const STEADY := 0.25
## How far off the target the sight may sit and still count as held on it, degrees.
const ON_TARGET := 1.5
## How high a beam rider holds the tube above the line of sight, degrees. A token angle, the way
## a real launcher leaves one: asked to gather five degrees it spends its first three hundred
## metres doing nothing else.
const RIDDEN_LIFT := 1.0
## Seconds to settle before a measured shot and to read the result after it.
const SETTLE_BEFORE := 0.8
const SETTLE_AFTER := 0.7
## Fall this far apart, in metres, before the trace records another point.
const TRACE_STEP := 1.5
...
var _row := -1
var _settle := 0.0
## Seconds the sight was on the target during the shot.
var _hold := 0.0
var _flight := 0.0
...
@onready var _side_camera: Camera3D = $Side
@onready var _trace: MeshInstance3D = $Trace
@onready var _body: CharacterBody3D = $Body
@onready var _tube: ShoulderLauncher = $Body/Eye/Launcher
...
func _ready() -> void:
	...
	_tube.refused.connect(_on_tube_refused)
	...
	match keycode:
		KEY_1:
			if _tube.round_scene != null:
				_flip_kind()
		KEY_2:
			_mark.speed = 0.0 if _mark.speed > 0.0 else 11.0
		KEY_3:
			_side = not _side
		KEY_4:
			var marks := _ranges()
			var step: int = marks.find(_tube.sight_range)
			_set_range(marks[(step + 1) % marks.size()])
		KEY_G:
			_table.clear()
			_row = 0
			_start_row()
...
## was built for.
func _set_range(dist: float) -> void:
	_tube.sight_range = dist
	_mark.place(Vector3(0.0, _floor, _stand.z - dist))
	if _tube.round_scene == null:
		return
...
func _pull() -> void:
	...
	shot.finished.connect(_on_round_finished)
	_shot = shot
	_flight = 0.0
	_hold = 0.0
...
func _look() -> void:
	...
	_side_camera.current = _side


func _walk(delta: float) -> void:
	var forward := (Input.get_action_strength(&"move_back")
			- Input.get_action_strength(&"move_forward"))
	var sideways := (Input.get_action_strength(&"move_right")
			- Input.get_action_strength(&"move_left"))
	var step := _body.global_basis.z * forward + _body.global_basis.x * sideways
...
func _feed(delta: float) -> void:
	...
	if _path.is_empty() or _path[-1].distance_to(_shot.global_position) > TRACE_STEP:
	...
	if to_mark.length_squared() > 1.0 and (-_eye.global_basis.z).angle_to(
			to_mark.normalized()) < deg_to_rad(ON_TARGET):
		_hold += delta


## POINT THE TUBE, its own step because it has to happen after the eye has moved and before the
## trigger, and those are not always the same moment. The tube is held ABOVE the line of sight;
## the BEAM stays on the sight, so a round leaves high and settles down onto the line.
##
## The tube is a CHILD of the eye, so `look_at` pins its LOCAL basis: turn the eye afterwards and
## the tube comes along still carrying the old elevation. In the measured run that matters — the
## `physics_frame` signal fires BEFORE `_physics_process`, so an `await` on it resumes a tick
## early, and the eye is swung onto the new range while the tube still holds the previous row's
## elevation. Two metres low at three hundred metres, which the ground turns into nine metres
## short. Same family as the rig-before-children ordering in the forty-fourth.
func _point() -> void:
...
## what the hand is actually doing. The whole cut scale for a thrown round, a token degree for a
## ridden one.
func _lift() -> float:
	return _tube.lift if _kind == ShoulderRound.Kind.DUMB else deg_to_rad(RIDDEN_LIFT)
...
func _aim_for_measurement() -> void:
	var to_mark := _mark.centre() - _eye.global_position
...
func _reset() -> void:
	_row = -1
	for child in _shots.get_children():
		child.queue_free()
	_shot = null
	_mark.reset()
	_tube.reset()
...
func _watch() -> void:
	...
	var side := (Vector3(-flat.z, 0.0, flat.x).normalized() if flat.length() > 1.0
			else Vector3.RIGHT)
	...
	_side_camera.global_position = mid + side * span * 0.78 + Vector3.UP * span * 0.14
	...
	_side_camera.look_at(mid, Vector3.UP)
...
## two-metre box is four pixels, and a trace is only readable next to something that says where
## it was supposed to end.
func _draw_mark(mesh: ImmediateMesh) -> void:
	var at := _mark.centre()
...
## Numbers that belong to a round in the air. With nothing flying they are the leftovers of the
## last shot, and printing them is the readout contradicting itself on one screen.
func _live(pattern: String, value: float) -> String:
	return (pattern % value) if _shot != null else "—"
...
func _readout() -> void:
	...
	var lines := PackedStringArray([
		...
		"последний пуск: %s" % (_last_shot if _last_shot != "" else "—"),
		"",
	...
	if not _table.is_empty():
		_append_table(lines, kind)
	_hud.text = "\n".join(lines)


func _append_table(lines: PackedStringArray, kind: String) -> void:
	lines.append("")
	...
	for row in _table:
		lines.append(
			"[cell]%.0f м[/cell][cell]%.1f с[/cell][cell]%s[/cell][cell]%.0f%%[/cell]"
			% [row["range"], row["time"],
				"[color=#7fe08a]в цель[/color]" if row["hit"]
					else ("%.1f м" % row["miss"] if row["miss"] < 900.0 else row["reason"]),
				row["hold"] * 100.0])
	lines.append("[/table]")


func _on_tube_refused(why: String) -> void:
	_last_shot = why


func _on_round_finished(miss: float, hit: bool, reason: String) -> void:
	_last_shot = "в цель" if hit else "%s, %.1f м" % [reason, miss]
	if hit:
		_mark.wreck()
	if _row >= 0:
		_table.append({
