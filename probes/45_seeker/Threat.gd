...
## the first version of the indicator looked like the absence of an indicator.
...
## Written by the rig every frame. Empty means there is no missile in the air.
var bearing := Vector3.ZERO   ## direction to the missile, in the target's own axes
var closing := 0.0
var locked := false
var live := false
...
func _draw() -> void:
	var mid := Vector2(RADIUS + 16.0, RADIUS + 16.0)
	...
	var away := Vector3.FORWARD.angle_to(dir) / PI
	...
	var tint := cold
	if locked:
		tint = hot if closing > 200.0 else warm
