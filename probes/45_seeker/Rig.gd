...
## window is how far off the boresight the target sits. Wide on the rail, where there is no
## seeker yet and a twenty-two degree crop of empty sky at three kilometres is a white box.
const EYE_FOV := 22.0
...
@onready var _eyecam: Camera3D = $Ui/Eye/View/Cam


func _ready() -> void:
	Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
	_dart.finished.connect(_on_finished)
	_dart.visible = false
...
func _reset_bodies() -> void:
	_prey.reset(Transform3D(Basis.IDENTITY, Vector3(0.0, 900.0, 0.0)))
	_prey.pitch_input = 0.0
	_prey.yaw_input = 0.0
...
func _shoot() -> void:
	...
	t.basis = Basis.looking_at(_prey.global_position - t.origin, Vector3.UP)
	_dart.launch(t, _prey)


func _on_finished(miss: float, hit: bool, reason: String) -> void:
	_last = "в цель" if hit else "%s, %.0f м" % [reason, miss]
	if _row >= 0:
		_table.append({
			"g": TEST_G[_row],
			"miss": miss,
...
func _eyeball() -> void:
	if _dart.flying:
		_eyecam.fov = EYE_FOV
		_eyecam.global_position = _dart.global_position
		_eyecam.look_at(_dart.global_position - _dart.global_basis.z * 100.0, Vector3.UP)
	else:
		_eyecam.fov = EYE_REST_FOV
		_eyecam.global_position = _rail.global_position
		_eyecam.look_at(_prey.global_position, Vector3.UP)
...
func _warn() -> void:
	...
	if not _dart.flying:
		return
	var d := _dart.global_position - _prey.global_position
	_threat.bearing = _prey.global_basis.inverse() * d
...
func _follow(delta: float) -> void:
	...
	var off := _prey.global_basis.z * chase_back + Vector3.UP * 8.0
	if _wide and _dart.flying:
		anchor = (_prey.global_position + _dart.global_position) * 0.5
		var span := maxf(_prey.global_position.distance_to(_dart.global_position), 120.0)
		off = Vector3(span * 0.9, span * 0.35, span * 0.9)
	_trail = off if _trail == Vector3.ZERO else _trail.lerp(off, clampf(delta * 3.0, 0.0, 1.0))
	_cam.global_position = anchor + _trail
	_cam.look_at(_prey.global_position, Vector3.UP)
	...
	match key.keycode:
		...
		KEY_1:
			_dart.head.law = (SeekerHead.Law.PURSUIT
				if _dart.head.law == SeekerHead.Law.PROPORTIONAL else SeekerHead.Law.PROPORTIONAL)
			_table.clear()
		KEY_2:
...
func _live(fmt: String, value: float) -> String:
	return (fmt % value) if _dart.flying else "—"


func _readout() -> void:
	var h := _dart.head
	var law := "погоня" if h.law == SeekerHead.Law.PURSUIT else "пропорциональное сближение"
	var lines := PackedStringArray([
		"[b]ГОЛОВКА НАВЕДЕНИЯ[/b]   закон: [b]%s[/b]   N = %.0f   предел ракеты %.0f g" % [
			law, h.gain, _dart.limit],
		"захват: %s   от оси %s   вид крутится %s   сближение %s" % [
			("[color=#7fe08a]есть[/color]" if h.locked else "[color=#ff8866]нет[/color]")
				if _dart.flying else "—",
			_live("%.0f°", rad_to_deg(h.off_bore)),
			_live("%.1f °/с", rad_to_deg(h.turn_rate)),
			_live("%.0f м/с", h.closing)],
		"просит %s   может %s   упиралась в предел %s" % [
			_live("[b]%.1f[/b] g", h.demand), _live("[b]%.1f[/b] g", _dart.pulled),
		...
		"дистанция %s   время %s%s" % [
			...
			"   последний пуск: " + _last if _last != "" else ""],
		"",
	...
	if not _table.is_empty():
		lines.append("")
		lines.append("[b]%s, N = %.0f, предел %.0f g[/b]" % [law, h.gain, _dart.limit])
		lines.append("[table=4][cell]цель тянет[/cell][cell]промах[/cell]"
			+ "[cell]время[/cell][cell]в пределе[/cell]")
		for r in _table:
			lines.append("[cell]%.0f g[/cell][cell]%s[/cell][cell]%.1f с[/cell][cell]%.0f%%[/cell]"
				% [r["g"], "[color=#7fe08a]в цель[/color]" if r["hit"]
					else ("%.0f м" % r["miss"] if r["miss"] < 9000.0 else r["reason"]),
					r["time"], r["saturated"] * 100.0])
		lines.append("[/table]")
	_hud.text = "\n".join(lines)
