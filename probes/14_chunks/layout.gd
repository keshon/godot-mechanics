...
func solve(library_of: Array, seed_value: int, target: int, extent: float, budget: int) -> bool:
	library = library_of
	...
	uses.resize(library_of.size())
	if library_of.is_empty():
		return false

	var first := _random.randi_range(0, library_of.size() - 1)
	if not _fits(_rect_of(library_of[first], Transform3D.IDENTITY)):
		return false
...
func _extend() -> bool:
	if _open.is_empty():
		return false
	var gi := _random.randi_range(0, _open.size() - 1)
	var step := {
...
func _options(gate: Dictionary) -> Array:
	var want: Transform3D = (
			(gate["xform"] as Transform3D)
			* Transform3D(Basis.from_euler(TURN), Vector3.ZERO)
	)
	var out: Array = []
	for chunk_index in library.size():
		var chunk: Dictionary = library[chunk_index]
		if uses[chunk_index] >= int(chunk["max_uses"]):
			continue
		var gates: Array = chunk["gates"]
		for gj in gates.size():
			if chunk["kinds"][gj] != gate["kind"]:
				continue
			tried += 1
			var placement: Transform3D = want * (gates[gj] as Transform3D).affine_inverse()
			if _fits(_rect_of(chunk, placement)):
				out.append([chunk_index, gj, placement])
...
func _seat(chunk_index: int, placement: Transform3D, skip_gate: int) -> void:
	var chunk: Dictionary = library[chunk_index]
	placed.append({"chunk": chunk_index, "xform": placement, "rect": _rect_of(chunk, placement)})
	uses[chunk_index] += 1
	var gates: Array = chunk["gates"]
	for i in gates.size():
		if i == skip_gate:
			continue
		_open.append({
			"xform": placement * (gates[i] as Transform3D),
			"kind": (chunk["kinds"] as PackedStringArray)[i],
		})


func _rect_of(chunk: Dictionary, placement: Transform3D) -> Rect2:
	var s: Vector2 = chunk["size"]
	var lo := Vector2(INF, INF)
	var hi := Vector2(-INF, -INF)
	for sx in [-0.5, 0.5]:
		for sz in [-0.5, 0.5]:
			var p: Vector3 = placement * Vector3(s.x * sx, 0.0, s.y * sz)
...
func _fits(rect: Rect2) -> bool:
	...
	if not _limit.encloses(rect):
		return false
	var test := rect.grow(-SLACK)
	for p in placed:
