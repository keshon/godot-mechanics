...
signal shattered(what: StuffSolid, born: Array[RigidBody3D], kept: float, ms: float)
...
##
## Семьсот, а не три сотни: пока в счёт шли ВСЕ тела, больше было нельзя, а теперь считается
## только нагрузка. На семистах обвал дома доламывает всё, что вообще может сломаться —
## целыми остаются ровно девять колонн первого этажа, которым падать некуда.
...
##
## Без этого обвал большого дома съедал бюджет за две секунды: две с половиной тысячи
## элементов давали семнадцать тысяч осколков.
...
## значит не должен мешать дому крошиться дальше.
##
## Уснувший и замороженный из счёта тоже выходят: решателю они не стоят ничего.
##
## Раньше здесь считались ВСЕ тела, и это была ошибка проектирования: настрелял три сотни
## обломков — и обвал дома перестаёт давать крошку, превращаясь в падение коробок. Бюджет
## обязан ограничивать НАГРУЗКУ, а не количество вещей на свете.
static var alive := 0
...
func _ready() -> void:
	...
	if walls.is_empty():
		var bs := ($Shape as CollisionShape3D).shape as BoxShape3D
		walls = StuffShatter.box(bs.size)
		volume = bs.size.x * bs.size.y * bs.size.z
	if stuff == null:
		return
	mass = maxf(volume * stuff.density, 0.05)
	physics_material_override = stuff.rubble
	($Mesh as MeshInstance3D).material_override = \
		stuff.shard_look if depth > 0 else stuff.body_look
...
func _physics_process(_delta: float) -> void:
	...
	var lost := 0.5 * mass * (_was_v.length_squared() - now.length_squared())
	var slow := _was_v.length() - now.length()
	var dir := _was_v.normalized() if _was_v.length_squared() > 1e-6 else Vector3.DOWN
	_was_v = now
	if lost > 1.0 and slow > 0.0:
		...
		# Сначала я поставил передачу внутрь `_bruise`, после проверки глубины, — и до остова
		# не доходило ничего, потому что падала на него как раз мелочь.
		_share(lost, mass * slow, dir)
		_bruise(lost, mass * slow, dir)
...
func _share(energy: float, push: float, dir: Vector3) -> void:
	...
	var q := PhysicsRayQueryParameters3D.create(global_position,
		global_position + dir * (reach * 2.0 + 0.4))
	q.exclude = [get_rid()]
	var r := get_world_3d().direct_space_state.intersect_ray(q)
	if r.is_empty():
		return
	var other := r["collider"] as StuffSolid
	if other != null and other != self:
		other.bruise(energy * passes_on, push * passes_on, dir)


## Принять чужой удар. Публично: бьёт не только мир, но и то, что в тебя прилетело.
func bruise(energy: float, push: float, dir: Vector3) -> void:
	_bruise(energy, push, dir)
...
func _bruise(energy: float, push: float, dir: Vector3) -> void:
	if depth >= mini(max_depth, impact_depth):
		return
	# БЮДЖЕТ ДЕЛИТСЯ, А НЕ ЗАКРЫВАЕТСЯ. Пока это были ворота «есть место — дроблю, нет —
	# не дроблю», обвал дома доставался тому, кто упал первым: плиты крошились (12 из 12),
	# а колонны приземлялись в закрытые ворота и оставались целыми (25 из 27) — при том что
	# порог у колонны НИЖЕ, то есть ломаться ей легче.
	#
	# Теперь остаток бюджета режет ЧИСЛО КУСКОВ, а не право на скол: поздние элементы
	# крошатся грубее, но крошатся все. Ступенька превратилась в наклон.
	# БЮДЖЕТ УРЕЗАЕТ ЧИСЛО КУСКОВ, А НЕ ЗАПРЕЩАЕТ СКОЛ. Пока он был воротами, обвал большого
	# дома не крошил НИЧЕГО: падающее здание — это тысячи живых тел, ворота захлопывались в тот
	# же миг, и колонны с плитами падали с двадцатого этажа целыми.
	#
	# Пол в две части: даже под самой тяжёлой нагрузкой предмет обязан треснуть хотя бы надвое.
	# Дешевле этого разрушения не бывает, а «совсем не сломалось» — это уже не бюджет, а обман.
	var room: int = maxi(budget - alive, LEAST)
	# Удар о мир задевает предмет целым боком, а не точкой: воронка во всю его половину.
	var reach0: float = pow(maxf(volume, 1e-6), 1.0 / 3.0) * 0.5
	soaked += stuff.soak(energy, volume, 4.18879 * reach0 * reach0 * reach0)
	var lim := limit()
	# Остаток бюджета режет число кусков ТОЛЬКО в этом сколе. Раньше он писался прямо в
	# `most`, и предмет, слегка стукнувшийся о землю, навсегда терял право дробиться мелко —
	# даже от ракеты потом.
	var cap: int = clampi(room, LEAST, most_on_impact)
	if soaked < lim or stuff.pieces(soaked, volume, cap) < 2:
		_wear(soaked / maxf(lim, 0.001))
		return
	...
	_blow.energy = energy
	_blow.momentum = push
	shatter(global_position + dir * reach * 0.5, dir, _blow, cap)
...
func hit(at: Vector3, dir: Vector3, shot: StuffRound) -> void:
	if _gone or stuff == null or shot == null:
		return
	if depth >= max_depth:
		apply_impulse(dir.normalized() * shot.momentum * 0.02, at - global_position)
		return
	soaked += stuff.soak(shot.energy, volume, shot.bite())
	var lim := limit()
	if soaked >= lim and stuff.pieces(soaked, volume, most) >= 2:
		shatter(at, dir, shot)
		return
	_wear(soaked / lim)


func shatter(at: Vector3, dir: Vector3, shot: StuffRound, cap := -1) -> void:
	if _gone:
		return
	_gone = true
	...
	if soaked <= 0.0:
		soaked = stuff.soak(shot.energy, volume, shot.bite())
	var count := stuff.pieces(soaked / maxf(toughness_scale, 0.01), volume,
		cap if cap > 0 else most)
	...
	# поверхности, а это формально снаружи: раньше каждый пятый выстрел молча переносил очаг
	# в ЦЕНТР предмета, и скол шёл не из дырки.
	var into: Vector3 = (global_transform.basis.inverse() * dir).normalized()
	var focus := StuffShatter.tuck(walls,
		global_transform.affine_inverse() * at + into * shot.calibre * 2.0)
	var grain := Vector3.ONE if grain_flat else stuff.grain
	# Мельче ребра своей крошки материал не дробится — это и задаёт шаг между зёрнами.
	# ВОРОНКА НЕ БЫВАЕТ МЕЛЬЧЕ СОБСТВЕННОЙ КРОШКИ МАТЕРИАЛА. Винтовочная воронка в три
	# сантиметра меньше бетонной щебёнки в двенадцать: зёрна в ней отбраковывались как тесные,
	# и вместо одиннадцати кусков выходило три — снова молча, без единого признака.
	var least: float = pow(stuff.grit, 1.0 / 3.0)
	var seeds := StuffShatter.spread(walls, count, focus,
		maxf(shot.crater(), least), _rng, least)
	var kept := 0.0
	var born: Array[RigidBody3D] = []
	for i in seeds.size():
		var c := StuffShatter.cell(walls, seeds, i, grain)
		...
		if c.is_empty() or float(c["volume"]) <= 0.0:
			continue
		kept += float(c["volume"])
		born.append(_spawn(c))
	_push(born, at, dir, shot)
	shattered.emit(self, born, kept / maxf(volume, 1e-6),
...
func _wear(part: float) -> void:
	...
	_mat.albedo_color = _mat.albedo_color.lerp(Color(0.2, 0.17, 0.15), part * 0.08)
...
func _push(born: Array[RigidBody3D], at: Vector3, dir: Vector3, shot: StuffRound) -> void:
	var crater: float = maxf(shot.crater(), 0.01)
	var share := PackedFloat32Array()
	var total := 0.0
	for b in born:
		var d: float = b.global_position.distance_to(at) / crater
		var k := 1.0 / (1.0 + d * d)
		share.append(k)
		total += k
	if total <= 0.0:
		return
	var ahead := dir.normalized()
	for i in born.size():
		var b := born[i]
		...
		b.apply_central_impulse(out.normalized() * shot.momentum * share[i] / total)


func _spawn(c: Dictionary) -> RigidBody3D:
	var body := SHARD.instantiate() as StuffSolid
	body.stuff = stuff
	body.walls = c["walls"]
	body.volume = c["volume"]
	body.depth = depth + 1
	body.most = most
